...
class AdaFisher(Optimizer):
    ...
    def __init__(self,
        ...
        self.diag_H: Dict[Module, Tensor] = {}
        self.diag_S: Dict[Module, Tensor] = {}
        self.tri_H: Dict[Module, Tensor] = {}
        self.tri_S: Dict[Module, Tensor] = {}
    ...
    def _save_input(self, module: Module, input: Tensor, output: Tensor):
        ...
        if is_grad_enabled() and self.steps % self.TCov == 0:
            diag_H = self.Cov_AMatrix(input[0].data, module)
            if self.steps == 0:
                self.diag_H[module] = diag_H.new(diag_H.size(0)).fill_(1)
            update_running_avg(MinMaxNormalization(diag_H), self.diag_H[module], self.gammas)
    ...
    def _save_grad_output(self, module: Module, grad_input: Tensor, grad_output: Tensor):
        ...
        if self.steps % self.TCov == 0:
            diag_S = self.Cov_GMatrix(grad_output[0].data, module)
            if self.steps == 0:
                self.diag_S[module] = diag_S.new(diag_S.size(0)).fill_(1)
            update_running_avg(MinMaxNormalization(diag_S), self.diag_S[module], self.gammas)
    ...
    def _get_update(self, module: Module):
        ...
        """
        # Fisher Computation
        F = kron(self.diag_H[module].unsqueeze(1), self.diag_S[module].unsqueeze(0)).t() + self.Lambda
        if module.bias is not None:
            F = [F[:, :-1], F[:, -1:]]
            F[0] = F[0].view(*module.weight.grad.data.size())
            F[1] = F[1].view(*module.bias.grad.data.size())
            return F
        else:
            return F.reshape(module.weight.grad.data.size())
    ...
    def _step(self, hyperparameters: Dict[str, float], param: Parameter, update: Tensor):
        ...
        (approximated by the Fisher information) and applies the update in a way that aims to
        ...
        - update (Tensor): The update tensor computed based on the Fisher information for the
                           parameter.
