...
class AdaFisherW(Optimizer):
   ...
   import torch
   model = MyModel()
   optimizer = AdaFisherW(model, lr=1e-3, beta=0.9, beta3=0.91, Lambda=1e-3, eps=1e-8, TCov=10, weight_decay=0)
   ...
   ```
    ...
    def _step(self, hyperparameters: Dict[str, float], param: Parameter, update: Tensor):
        ...
        - hyperparameters (dict): A dictionary containing optimization hyperparameters, including
                                  'lr' for learning rate, 'betas' for the exponential moving average
                                  coefficients, and 'eps' for numerical stability.
        ...
        denom = ((exp_avg_Fisher ** 0.25) /
                 (bias_correction2 ** 0.25)).add_(hyperparameters['eps'])
        param.data -= hyperparameters['lr'] * (exp_avg / bias_correction1 / denom + hyperparameters['weight_decay']
                                               * param.data)
